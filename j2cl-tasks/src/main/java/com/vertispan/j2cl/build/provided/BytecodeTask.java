...
public class BytecodeTask extends TaskFactory {
    ...
    public Task resolve(Project project, Config config, BuildService service) {
        ...
        if (!project.hasSourcesMapped()) {
            ...
            Input existingUnpackedBytecode = input(project, OutputTypes.INPUT_SOURCES, service);
            return context -> {
        ...
        return context -> {
            if (!inputSources.getFilesAndHashes().isEmpty()) {
                ...
                if (incremental) { //TODO DO WE NEED IT
