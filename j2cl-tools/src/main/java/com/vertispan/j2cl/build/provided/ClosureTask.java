...
public class ClosureTask extends TaskFactory {
    ...
    public Task resolve(Project project, Config config, BuildService buildService) {
        ...
        List<Input> jsSources = Stream.concat(
                Stream.of(
                        input(project, OutputTypes.TRANSPILED_JS, buildService).filter(JS_SOURCES),
                        input(project, OutputTypes.GENERATED_SOURCES, buildService).filter(PLAIN_JS_SOURCES),
                        input(project, OutputTypes.INPUT_SOURCES, buildService).filter(PLAIN_JS_SOURCES)
                ),
                scope(project.getDependencies(), Dependency.Scope.RUNTIME)
                .stream()
                .flatMap(p -> Stream.of(
                        input(p, OutputTypes.TRANSPILED_JS, buildService).filter(JS_SOURCES),
                        input(p, OutputTypes.GENERATED_SOURCES, buildService).filter(PLAIN_JS_SOURCES),
                        input(p, OutputTypes.INPUT_SOURCES, buildService).filter(PLAIN_JS_SOURCES)
                ))
